...
leading.blanks <-  function(df, blank.char = ".", extra.char = " ", min.width = 1, max.width = 99, ignore.first = FALSE, trace = FALSE){
  ...
  for(i in 1:ncol(df)) {
    ...
    cc.width <- nchar(as.character(cc),type="width") #S-plus: string.bounding.box(cc)$columns
    cc.max.width <- min(max.width, max(cc.width, min.width))
    cc.pure <- paste(rep(blank.char, cc.max.width), sep = "", collapse = "")
    if(trace)
...
dayno.calc <- function(Tday="31.01.2001",Ttime="00:00",ref.date="31.12.2000",ref.time="00:00"){
  ...
  dayno.ref <- as.integer(chron::dates(base::as.character(ref.date),format="d.m.y"))+as.double(dayno.clock(ref.time))
  dayno.day <- as.integer(chron::dates(base::as.character(Tday),format="d.m.y"))-dayno.ref
